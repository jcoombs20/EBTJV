			...
			if vaaArray[vaaIndex][4] != 56600:									
				try:								
					...
					try:							
						...
						if cpArray[cpInd][6] == "Yes" or cpArray[cpInd][7] == 0 or cpArray[cpInd][2][0] != "1":						
...
for catchRow in sorted(catchRecords, key=lambda k: (k[0], -k[2]), reverse=True):
	...
	try:
		...
		#if cpArray[cpIndex][2] != "0" and cpArray[cpIndex][2] != "0P" and cpArray[cpIndex][2] != "0.5" and cpArray[cpIndex][2] != "0.5P" and tmpBi == 0:
		if cpArray[cpIndex][2][0] == "1":
